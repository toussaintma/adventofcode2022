...
def deduce_shape(opponent, you):
  base = "ACB" 
  shape = opponent
  opp_index = base.find(opponent)
  if you == "A": #lose
    if opp_index + 1 == len(base):
      shape = base[0]
    else:
      shape = base[opp_index + 1]
  elif you == "B": #draw
    shape = opponent
  elif you == "C": #win
    if opp_index - 1 < 0:
      shape = base[len(base) - 1]
    else:
      shape = base[opp_index - 1]
  print('deducing:', opponent, you, 'gives', opp_index, shape)
  return shape
