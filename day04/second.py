def is_fully_contained(sections):
  ...
  # case left_side then right_side
  if int(left_side[0]) <= int(right_side[0]) and int(right_side[0]) <= int(left_side[1]):
    contained = True
  
  # case right_side then left_side
  if int(right_side[0]) <= int(left_side[0]) and int(left_side[0]) <= int(right_side[1]):
    contained = True
